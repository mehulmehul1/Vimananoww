...
function layoutTextOnSegments(text: string, segments: LineSegment[], fontSize: number): SegmentMapping {
...
export const AssetGenerator: React.FC = () => {
  ...
  const regenerate = useCallback((time: number, fullLayout: boolean = true) => {
    ...
    if (result.type === 'segments') {
      ...
      if (fullLayout) {
        const mapped = layoutTextOnSegments(text, result.segments, fontSize);
